...
class Sharing extends ApiBase
{
    ...
    public function del_comment()
    {
        ...
        if(!$id){
            $this->ajaxReturn(['status' => -1 , 'msg'=>'请提供评论或者回复id','data'=>'']);
        }
     
        $res = Db::name('sharing_comment')->where(['user_id'=>$user_id,'id'=>$id])->delete();
        if($res){
            Db::name('sharing_circle')->where('id',$sharing_id)->setDec('comment_num',1);
    ...
    public function my_user()
    {
        ...
        $user = Db::name('member')->field('id,nickname,avatar,user_diy_background')->where('id',$user_id)->find();
